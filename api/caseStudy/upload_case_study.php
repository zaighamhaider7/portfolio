...
// Validate
if (empty($title) || empty($description) || !$imagePath || empty($sort_order)) {
    echo json_encode(["success" => false, "message" => "Missing title, description, or image"]);
    exit();
}

// Insert into DB
$sql = "INSERT INTO casestudy (title, description, image,sort_order) VALUES (?, ?, ?,?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ssss", $title, $description, $imagePath,$sort_order);
