...
export default function websiteView() {
  ...
  const fetchEntries = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/getWeb.php`);
  ...
  const submitEdit = async () => {
    ...
    fd.append('web_link', editData.web_link);
    if (editData.image instanceof File) {
      fd.append('image', editData.image);
    }

    try {
      const res = await fetch(`${BASE_URL}/web_update.php`, { method: 'POST', body: fd });
      const json = await res.json();
      if (json.success) {
        setIsEditOpen(false);
        fetchEntries();
  ...
  const handleDelete = (id) => {
    ...
    fetch(`${BASE_URL}/web_delete.php?id=${id}`)
      .then(res => res.json())
      .then(() => fetchEntries())
      .catch(console.error);
  };
  ...
  return (
    <Box p={3}>
      <Typography variant="h5" gutterBottom>Uploaded Website Designs</Typography>
      ...
      <Dialog open={isEditOpen} onClose={() => setIsEditOpen(false)} fullWidth maxWidth="sm" ref={dialogRef}>
        ...
        <DialogContent dividers>
          <Stack spacing={2}>
            ...
            <TextField label="Website Link" fullWidth value={editData.web_link} onChange={(e) => setEditData({ ...editData, web_link: e.target.value })} />
            <TextField label="Sort Order" fullWidth value={editData.sort_order} onChange={(e) => setEditData({ ...editData, sort_order: e.target.value })} />
            ...
            <TableBody>
              {entries.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell><a href={row.web_link}>Link</a></TableCell>
                  <TableCell>
