...
import {
  ...
  Divider, TextField, Button, Stack
} from '@mui/material';
import CloudUploadIcon from '@mui/icons-material/CloudUpload';

const AppDev = () => {
  // const [formData, setFormData] = useState({
  //   overview: '', title: '', description: '', heading1: '', paragraph1: '',
  //   goal: '', textSection: '', text3: ''
  // });
  ...
  const [files, setFiles] = useState({
    app_banner_image: { file: null, preview: '' },
    ...
    app_front_image: { file: null, preview: '' }
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('http://localhost/materially-free-react-admin-template-1.0.0/materially-free-react-admin-template-1.0.0/api/upload_mobile_app.php', {
        method: 'POST',
        body: data
      });
    ...
    <Grid container spacing={3} justifyContent="center">
      <Grid item xs={12} lg={5}>
        <Card elevation={2} className="rounded-3xl shadow-md">
          <CardHeader title={<Typography variant="h6" fontWeight={600}>Upload Mobile App Development Page</Typography>} />
          <Divider />
          <CardContent>
            <Stack spacing={2}>

              <Typography variant="subtitle1" fontWeight={600}>App Title & Description</Typography>
              <TextField fullWidth label="App Title" value={formData.app_title} onChange={handleText('app_title')} />
              <TextField fullWidth label="App Title Description" value={formData.app_title_description} onChange={handleText('app_title_description')} />
              <Button component="label" variant="outlined" startIcon={<CloudUploadIcon />}>
                App Front Image
                <input hidden accept="image/*" type="file" onChange={handleFile('app_front_image')} />
              </Button>
              {renderPreview('app_front_image')}

              <Divider />

              <Typography variant="subtitle1" fontWeight={600}>Hero Section</Typography>
              <TextField fullWidth label="App Name" value={formData.app_name} onChange={handleText('app_name')} />
              <TextField fullWidth label="App Description" value={formData.app_description} onChange={handleText('app_description')} />
              <Button component="label" variant="outlined" startIcon={<CloudUploadIcon />}>
                Upload Banner Image
                <input hidden accept="image/*" type="file" onChange={handleFile('app_banner_image')} />
              </Button>
              {renderPreview('app_banner_image')}
